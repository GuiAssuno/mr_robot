...
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  if (type == WStype_TEXT) {
    // payload é a mensagem que chegou (ex: "F")
    char cmd = (char)payload[0];
    
    // Atualiza a variável compartilhada (Memória RAM)
    // O Núcleo 0 vai ler isso quase instantaneamente
    comandoWeb = cmd; 
  }
}
...
void loop() {
    ...
    server.handleClient();
    M_distancia[x][0] = distancia_direita.dist();
    M_distancia[y][1] = distancia_esquerda.dist();
    y++;
    x++; 
...
void autosafe() {
  ...
  if ((esq < safe) || (dir < safe)) {
    ...
    if (esq < safe) digitalWrite(ledSonarEsquerdo, HIGH);
    else digitalWrite(ledSonarEsquerdo, LOW);

    if (dir < safe) digitalWrite(ledSonarDireito, HIGH);
    else digitalWrite(ledSonarDireito, LOW);

    if ((esq < safe) && (dir < safe)){       
        if (!liberado){ // se não liberado
            flag = true;
            stop();
            return;
        }
        else { // se não, ou seja se liberado
            flag = false;
            stop();
            return;
            } 
...
void comando(char inst) {
    switch (inst) {
      case 'F': funcao = &Frente(); break;
      case 'B': funcao = &Back(); break;
      case 'S': Parar(); break;
      case 'L':
        press_l = true;
        if (modo) Esquerda();
...
void Frente() {
    ...
    if (millis() >= espera) {
        analogWrite(pwm_direito, velo_atual);
        analogWrite(pwm_esquerdo, velo_atual);
        velo_atual += marcha;
