...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .btn-stop {background: #dc3545; color: white; border-color: #b02a37; transition: background-color 0.3s;}
        .btn-stop.livre {background: #28a745 !important; border-color: #198754 !important;
        }
    ...
    <div class="container">        
        ...
        <div class="controls-section">
            ...
            <div class="side-controls">
                ...
                <div class="control-label" id="lblLock">TRAVADO</div>
...
<script>
    ...
    var estadoTravado = true; 
    var estadoGiro360 = false;
    var ws;
    var lastSend = 0;
    ...
    function setupJoystick(containerId, knobId, type) {
        ...
        function reset() {
            ...
            // Zera o valor deste joystick específico
            joyState[type] = { x: 0, y: 0 };
            
            // LÓGICA DE PARADA IMEDIATA
            if (window.innerHeight < window.innerWidth && window.innerWidth < 900) {
                 // Modo Paisagem (Dois Joysticks)
                 // Quando solta um, ele manda 0 para aquele eixo, 
                 // mas mantém o valor do outro eixo se o outro dedo ainda estiver lá.
                 sendJoy(joyState.right.x, joyState.left.y);
            } else {
                 // Modo Retrato/Desktop (Um Joystick)
                 // Soltou? Manda 0,0 (PARAR TUDO)
                 sendJoy(0, 0);
            }
        }
    ...
    function atualizarVisualTrava() {
        // Pega todos os botões de trava (Desktop e Mobile)
        var btns = [document.getElementById('btnStop'), document.getElementById('btnStopPortrait'), document.getElementById('btnStopLandscape')];
        // Pega o label (texto embaixo) - Só o do Desktop tem label visível no seu layout original, mas atualizamos se houver
        var lbl = document.getElementById('lblLock');

        btns.forEach(btn => {
            if (!btn) return;
            if (estadoTravado) {
                // Modo TRAVADO: Vermelho, Texto X ou Cadeado
                btn.classList.remove('livre'); // Remove o verde
                btn.innerText = "X"; // Ou icone de cadeado fechado
            } else {
                // Modo LIVRE: Verde
                btn.classList.add('livre'); // Adiciona o verde
                btn.innerText = "OK"; // Ou icone de cadeado aberto
            }
        });

        if (lbl) {
            lbl.innerText = estadoTravado ? "TRAVADO" : "LIVRE";
            lbl.style.color = estadoTravado ? "#dc3545" : "#28a745"; // Muda cor do texto também
        }
    }

    // Função para atualizar visual do MODO
    function atualizarVisualModo() {
        var lbl = document.getElementById('modeText'); // Texto lá no topo
        var texto = estadoGiro360 ? "GIRO" : "CURVA";
        if (lbl) lbl.innerText = texto;
    }

    // Funções de Clique (Substituem os onClicks antigos)
    function toggleTrava() {
        estadoTravado = !estadoTravado; // Inverte
        send('X'); // Manda comando pro robô
        atualizarVisualTrava(); // Atualiza tela
    }

    function toggleModo() {
        estadoGiro360 = !estadoGiro360; // Inverte
        send('A'); // Manda comando pro robô
        atualizarVisualModo(); // Atualiza tela
    }
    ...
    window.onload = function() {
        ...
        if(savedIP) {
            ...
            document.getElementById('cameraStreamDesktop').src = savedIP;
            document.getElementById('videoStatusDesktop').style.display = 'none'; // Esconde status se carregar
            document.getElementById('cameraStreamPortrait').src = savedIP;
            document.getElementById('cameraStreamLandscape').src = savedIP;
        ...
        function clickLink(id, cmd) {
            var setupBtn = function(id, func) {
            var el = document.getElementById(id);
            if(el) el.onclick = func;
            };

            setupBtn('btnMode', toggleModo);
            setupBtn('btnStop', toggleTrava);
            
            setupBtn('btnModePortrait', toggleModo);
            setupBtn('btnStopPortrait', toggleTrava);
            
            setupBtn('btnModeLandscape', toggleModo);
            setupBtn('btnStopLandscape', toggleTrava);
